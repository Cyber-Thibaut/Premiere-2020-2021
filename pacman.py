...
def prebuild():
    ...
    draw_string("Spider-Man\n   Creation", 20, 180, colors[3], colors[0])
def render():
        ...
        while sum(pacgommes) + sum(superpacgommes):
            ...
            while monotonic() - depart < 0.01:
                ...
                if pacman.move() == 42:
                    ...
                    print("Tu a perdu au niveau {} avec un\nscore de {}".format(lvl,score))
                    return 69
                if keydown(17):
                    draw_string("PERDU",185,100,colors[3],colors[0])
                    print("Tu a appuye sur le bouton INTERDIT")
                    print("Tu a perdu au niveau {} avec un\nscore de {}. Il te restait {} vie(s)".format(lvl,score,lives))
                    return 69
            draw_string(str(score),70-5*len(str(score)),60,(255,)*3,colors[0])
            if frightened: #Fantomes
